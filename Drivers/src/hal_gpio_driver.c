...
void hal_gpio_set_alt_function(GPIO_TypeDef  *GPIOx, uint16_t pin_no, uint16_t alt_fun_val) {
    if ( pin_no <= 7 ) {
        GPIOx->AFR[0] &= ~(0x15 << ( 4 * pin_no ) );
        GPIOx->AFR[0] |= (alt_fun_val << ( pin_no * 4 ));
    } else {
        GPIOx->AFR[1] &= ~(0x15 << ( 4 * ( pin_no % 8)));
...
uint8_t hal_gpio_read_from_pin(GPIO_TypeDef  *GPIOx, uint16_t pin_no) {
    uint8_t value;
    /* Read from the IDR register then right shift by the value of pin number to get the input status value of the pin to lsb */
...
void hal_gpio_set_pin_as_interrupt(uint16_t pin_no, uint8_t port_select) {
    SYSCFG->EXTICR[pin_no/4] &= ~(0x15 << ( 4 * pin_no ) );
    SYSCFG->EXTICR[pin_no/4] &= ~(port_select << ( 4 * pin_no ) );
}
