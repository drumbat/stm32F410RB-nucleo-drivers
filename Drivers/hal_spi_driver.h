...
#define SPI_REG_CR2_EREIE_ENABLE       ( (uint32_t) 1 << 5 )
...
typedef struct {
    ...
    uint32_t CLKPolarity;       /* Specifies serial clock steady state. */
    uint32_t CLKPhase;          /* Specifies the clock active edge for bit capture. */
...
typedef struct __spi_handle_t {
    ...
    uint8_t            *pRxBuffPtr;     /* Pointer to SPI Tx transfer Buffer */
...
* @param     hspi: pointer to an spi_handle_t structur that
